...
def evaluate(normalized_dir: Path, exact_path: Path, additional_path: Path, truth_path: Path) -> dict:
    ...
    channels.update(_channels(additional_path))
    counts = Counter()
    with truth_path.open(encoding="utf-8", newline="") as file:
        ...
        for s1_id, raw_targets in reader:
            ...
            for target_id in raw_targets.split(",") if raw_targets else []:
                ...
                if hits:
                    counts["all_cheap_union_gt_pairs"] += 1
    ...
    result["conditional_cheap_union_pair_recall"] = counts["all_cheap_union_gt_pairs"] / eligible if eligible else None
    return result
