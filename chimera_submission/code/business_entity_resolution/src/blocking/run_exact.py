"""Build/reuse exact indexes and produce bounded channel candidate Parquet."""
...
from chimera_submission.code.business_entity_resolution.src.blocking.exact import build_and_retrieve
from chimera_submission.code.business_entity_resolution.src.config import load_config


def main() -> int:
    parser = argparse.ArgumentParser(description="Run exact clean/core name blocking")
    ...
    parser.add_argument("--per-channel-cap", type=int, default=30)
    ...
    index_path = root / "indexes" / sample_dir / f"{args.split}_exact.pkl"
    candidate_path = root / "blocking" / sample_dir / f"{args.split}_exact.parquet"
    report = build_and_retrieve(target_paths, query_path, index_path, candidate_path,
                                args.max_block_size, args.per_channel_cap)
