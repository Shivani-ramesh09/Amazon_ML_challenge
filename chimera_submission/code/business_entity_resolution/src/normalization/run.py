...
def _normalizer_signature(input_path: Path, modulus: int, truth_path: Path | None = None) -> str:
    ...
    if truth_path is not None:
        digest.update(_sha256(truth_path).encode())
    for name in ("text.py", "address.py", "run.py"):
        digest.update(_sha256(Path(__file__).with_name(name)).encode())
    return digest.hexdigest()


def linked_target_ids(truth_path: Path, sample_modulus: int) -> set[str]:
    """Complete the dev target universe for selected train S1 entities."""
    linked = set()
    with truth_path.open(encoding="utf-8", newline="") as file:
        ...
        for s1_id, raw_targets in reader:
            if int(s1_id.partition("-")[2]) % sample_modulus == 0 and raw_targets:
                linked.update(raw_targets.split(","))
    return linked


def normalize_file(path: Path, output: Path, *, sample_modulus: int = 1,
                   include_entity_ids: set[str] | None = None, compression: str = "zstd") -> dict:
    if sample_modulus < 1:
    ...
    try:
        ...
        for batch in reader:
            ...
            for entity_id, name, address, country in zip(*(values[key] for key in INPUT_COLUMNS)):
                scanned += 1
                if sample_modulus > 1 and int(entity_id.partition("-")[2]) % sample_modulus and (
                    include_entity_ids is None or entity_id not in include_entity_ids):
                    continue
    ...
    return {
        ...
        "linked_target_ids": len(include_entity_ids) if include_entity_ids is not None else 0,
...
def main() -> int:
    ...
    truth_path = (Path(config["data_dir"]) / "train" / "train_ground_truth.tsv"
                  if args.split == "train" and args.source in (2, 3) and modulus > 1 else None)
    ...
    signature = _normalizer_signature(input_path, modulus, truth_path)
    if output_path.is_file() and manifest_path.is_file():
        cached = json.loads(manifest_path.read_text())
        if cached.get("signature") == signature:
            print(json.dumps({"cache_hit": True, **cached}, indent=2))
            return 0
    linked = linked_target_ids(truth_path, modulus) if truth_path is not None else None
    report = normalize_file(input_path, output_path, sample_modulus=modulus, include_entity_ids=linked)
